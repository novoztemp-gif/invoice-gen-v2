...
export async function GET(request: NextRequest) {
  ...
  if (batchError || !batch) {
    return NextResponse.json(
      { message: "Batch not found", error: batchError },
      { status: 404 },
    );
  }
  ...
  if (invoices.length === 0) {
    return NextResponse.json(
      { message: "No invoices generated for this batch" },
      { status: 400 },
    );
  }
  ...
  for (const inv of invoices) {
    ...
    if (isPurchase) {
      ...
      zip.file(`${inv.invoice_number}.pdf`, pdfBuffer);
    } else {
      ...
      zip.file(`${inv.invoice_number}.xlsx`, excelBuffer);
    }
  }
  ...
  return new NextResponse(zipBuffer as any, {
    status: 200,
    headers: {
      "Content-Disposition": `attachment; filename="Invoice_Batch_${batch.financial_year}.zip"`,
