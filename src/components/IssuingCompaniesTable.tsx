...
import { EditIssuingCompanyDialog } from "./EditIssuingCompanyDialog";
...
type SortField = keyof IssuingCompany;
type SortOrder = "asc" | "desc" | null;

export function IssuingCompaniesTable({
  companies,
}: {
  ...
  const [editingCompany, setEditingCompany] = useState<IssuingCompany | null>(
    null,
  );
  ...
  const filteredAndSortedCompanies = useMemo(() => {
    let filtered = companies.filter((company) => {
      ...
      return (
        ...
        company.branch?.toLowerCase().includes(query) ||
        company.bank_account_name.toLowerCase().includes(query) ||
        company.bank_name.toLowerCase().includes(query)
      );
    });
      ...
      {filteredAndSortedCompanies.length === 0 ? (
        <p className="text-slate-500 text-center py-8">
          {searchQuery
            ...
            : "No companies found. Add your first issuing company!"}
        </p>
      ) : (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="min-w-[130px]">Branch</TableHead>
                <TableHead className="min-w-[180px]">
                  Bank Account Name
                </TableHead>
                <TableHead className="min-w-[180px]">Bank Name</TableHead>
                <TableHead className="min-w-[160px]">Account Number</TableHead>
            ...
            <TableBody>
              {filteredAndSortedCompanies.map((company) => (
                <TableRow key={company.id}>
                  ...
                  <TableCell>{company.bank_account_name}</TableCell>
