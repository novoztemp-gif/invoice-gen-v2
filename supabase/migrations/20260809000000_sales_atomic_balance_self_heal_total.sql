-- The RPC's own "Batch Total Mismatch" check compared the real sum of all
-- invoices against invoice_batch.total_amount — a column set once at batch
-- generation time and never kept in sync afterward. The TypeScript engine
-- (SalesFinalValidator) already guarantees, before this RPC is ever called,
-- that THIS edit conserves whatever the batch's invoices already summed to
-- — so if the stored column disagrees with reality, that's stale/incorrect
-- data from generation, not something this edit introduced. Rejecting the
-- edit outright over pre-existing drift the edit had nothing to do with
-- makes every future edit to that batch permanently impossible. Self-heal
-- instead: sync the stored total to match reality after a successful,
-- already-validated edit, the same way an edit is allowed to fix
-- already-out-of-bounds rates/quantities on lines it didn't touch.
...
BEGIN
  ...
  INSERT INTO public.sales_batch_edit_audit (
    ...
    auth.uid(),
    NOW(),
    (SELECT jsonb_agg(v_item->>'id') FROM jsonb_array_elements(
