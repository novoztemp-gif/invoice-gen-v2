-- Sales Phase 1 extension: atomically persist a sales invoice edit, all
-- balancing updates, AND any brand-new invoices SalesNewInvoiceCreator had
-- to create as a last resort (when no existing invoice had room to absorb
-- a product's quantity surplus). Inserting these in the same transaction
-- as the rest of the edit keeps the whole operation atomic — either
-- everything lands, or nothing does. Mirrors
-- 20260807000000_purchase_atomic_balance_new_invoices.sql for the Sales
-- RPC (20260728000000_sales_atomic_balance_rpc.sql).
