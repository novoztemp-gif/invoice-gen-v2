...
CREATE TABLE IF NOT EXISTS public.daily_stock_ledger (
    ...
    purchase_batch_id UUID REFERENCES public.invoice_batch(id) ON DELETE CASCADE,
    invoice_batch_id UUID REFERENCES public.invoice_batch(id) ON DELETE CASCADE,
    ...
    closing_stock NUMERIC(15, 2) DEFAULT 0 NOT NULL,
    created_at TIMESTAMPTZ DEFAULT now() NOT NULL,
    updated_at TIMESTAMPTZ DEFAULT now() NOT NULL,
    CONSTRAINT unique_batch_date_product UNIQUE (invoice_batch_id, ledger_date, product_id)
);
